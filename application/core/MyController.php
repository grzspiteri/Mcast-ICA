...
class MY_Controller extends CI_Controller {
    ...
    protected function check_login()
    {
        ...
        # 2. Stop here if there is no session database
    ...
    protected function has_permission($p_name)
    {
        ...
        $permission = $this->config->item('permission')[$role];

        # 3. check that the permission item actually exists
        if (!array_key_exists($p_name, $permission)) return FALSE;
        return $permission[$p_name];
    }
